...
const std::map<DiffConfig, std::vector<double>> diff_coeffs = {
...
double derivative (const std::function<double(std::vector<double> &)> &f,
                       ...
                       uint64_t idx,
                       DiffConfig diff) {
    std::vector<double> args(X); // Создаем копию вектора X, чтобы не менять оригинал
    double ans = 0; // Значение производной, которое будем вычислять
...
double derivative (const std::function<double(double)> &f,
                       ...
                       double h,
                       DiffConfig diff) {
    double arg = x; // Создаем копию переменной x
    double ans = 0; // Значение производной, которое будем вычислять
    const auto &coeffs = diff_coeffs.find(diff)->second; // Вектор коэффициентов, который соответствует заданной конфигурации
    uint64_t size = coeffs.size(); // Размер вектора коэффициентов
    uint64_t points = (size - 1) / 2; // Количество точек на сетке, которые будут использоваться при вычислении производной
