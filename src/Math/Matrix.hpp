...
class Matrix {
    public:
        ...
        std::vector<T> toVector (uint64_t start, uint64_t end) const;
...
template <class T>
std::vector<T> Matrix<T>::toVector (uint64_t start, uint64_t end) const {
    return std::vector<T>(buff.cbegin() + start, buff.cbegin() + end);
}
