...
//std::vector<std::function<float128_t (const std::vector<float128_t> &)>>
...
float128_t ToughCoeff (const std::vector<std::function<float128_t (const std::vector<float128_t> &)>> &system, const std::vector<float128_t> &Y) {
    float128_t coeff = 0, curr_coeff;
    uint64_t order = system.size();
    Matrix<float128_t> A(order, order);
    //std::vector<float128_t> args(order + 1, 0);
    //args[0] = x;
    ...
    for (uint64_t i = 0; i < order; ++i) {
        for (uint64_t j = 0; j < order; ++j) {
            A(j, i) = derivative(system[j], Y, 0.001, i + 1, DiffConfig::POINTS4_ORDER1_WAY2);
        }
    }
    std::cout << "matrix:\n" << A << "\n";
    //std::cout << "Yakobi:\n" << A << "\n";
    // Matrix<float128_t> L, U, P;
    // float128_t new_det = 1;
    // std::tie(L, U, P) = LU(A);
    // for (uint64_t i = 0; i < U.size().n; ++i) {
    //     new_det *= U(i, i);
    // }
    std::vector<float128_t> lambdas = QRFindLambda(A, float128_t(0.001));
    float128_t max = *std::max_element(lambdas.begin(), lambdas.end());
    float128_t min = *std::min_element(lambdas.begin(), lambdas.end());
            //if (max >= 0 || min >= 0) {
            //    curr_coeff = 0;
            //} else {
                if (max == min) {
    ...
    A = Matrix<float128_t>(6, 6, {-15.4673900864288,       -2696504.45348263,       -5.15579669547631,       -37.4167682121373,       -5.15579669547620,       -2164792.68116993,     
  ...
  -914761.311360175,        2696509.60927932,      -914771.617434723,        10.3115933909526,       -914771.622953566,        2164797.83696663,
   -4.329575050746476E-014,  6.008593542900178E-014, -7.309394923668382E-015,  1.829563869093914E-014,   914781.934546957,       -2164787.52537324});
    lambdas = QRFindLambda(A, float128_t(0.001));
...
// float128_t ToughCoeff (const Task &task, const std::vector<float128_t> &Y, float128_t x) {
//     float128_t coeff = 0, curr_coeff;
...
//         args[i] = task.Y[i];
//     }
//     float128_t step = (task.Xn - task.X0) / COUNT_OF_STEPS;
//     float128_t det = 0;
//     for (float128_t x = task.X0; x <= task.Xn; x += step) {
//         args[0] = x;
//         for (uint64_t i = 0; i < order; ++i) {
//             //args[i + 1] = 1;
//             for (uint64_t j = 0; j < order; ++j) {
...
//         // }
//         //std::cout << "\ncurrent A:\n" << A << "\n";
//         //std::vector<float128_t> lambdas = QRFindLambda(A, 0.01);
//         //std::cout << "lambdas: ";
//         //printVector(lambdas);
//         //float128_t max = *std::max_element(lambdas.begin(), lambdas.end());
//         //float128_t min = *std::min_element(lambdas.begin(), lambdas.end());
