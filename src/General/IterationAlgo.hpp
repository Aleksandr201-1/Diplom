...
std::vector<std::vector<double>> IterationStep (const std::vector<std::vector<double>> &K, const std::vector<std::vector<double>> &Yi, uint64_t idx, const std::vector<std::function<double (const std::vector<double> &)>> &f, const Matrix<double> &butcher, double h, double approx, IterationAlgo algo);
